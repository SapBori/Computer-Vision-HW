...
KImageGray& KImageGray::HistoMatching(KImageGray& igSource, KImageGray& igTarget, KImageGray de){
    double             source_npHisto[256] = { 0,  };
    int             source_upAdd[256] ={0,};
    int             source_Size = igSource.Size();
    //double          source_pr[256] = {0,};

    for(int i =0; i<igSource.Row();i++){
        for(int j=0; j<igSource.Col();j++){
            source_upAdd[igSource[i][j]]++;
        }
    }

    for(int a=0; a<256; a++){
        source_npHisto[a]= source_upAdd[a]/source_Size;
    }
    //source_pr[0]= (source_npHisto[0]/source_Size);
    //source's accumulated histogram
    for(int i=1, ic=255; ic; i++, ic--){
        source_npHisto[i] += source_npHisto[i-1];
        //source_pr[i]= source_npHisto[i]/source_Size;
    }

    double          target_npHisto[256] = { 0,  };
    int             target_upAdd[256] ={0,};
    int             target_Size = igTarget.Size();
    //double          target_pr[256] = {0,};
    for(int i =0; i<igTarget.Row();i++){
        for(int j=0; j<igTarget.Col();j++){
            target_upAdd[igTarget[i][j]]++;
        }
    }
    for(int a=0; a<256; a++){
        target_npHisto[a]=target_upAdd[a]/target_Size;
    }
    //target_pr[0]= (target_npHisto[0]/target_Size);
    //target's accumulated histogram
    for(int i=1, ic=255; ic; i++, ic--){
        target_npHisto[i] += target_npHisto[i-1];
        //target_pr[i]= target_npHisto[i]/target_Size;
    }
    int table[256]={0,};
    int pos=0;

    // Intencity Matching
    for(int i=0, ic=255; ic; i++, ic--){
        double dif=0,min=300000;
        for(int j=pos,jc=255; jc; j++,jc--){
            dif = _ABS(source_npHisto[i]-target_npHisto[j]);
            if(min>dif){
               min=dif;
               pos=j;
            }
    ...
    for(int p=0, pc=255; pc; p++, pc--){
       int ch = 0;
       ch = table[p];
       for(int i =0; i<de.Row(); i++){
           for(int j =0; j<de.Col(); j++){
               if(igSource._ppA[i][j]==p)
                   igSource._ppA[i][j]=ch;
               else continue;
           }
       }
    }
}
KImageColor& KImageColor::SPNoise(KImageColor &igSource){
    for(int i =0 ;i<(igSource.Size()/20);i++){
        // Salt-Pepper 이미지가 나올 확률 1/20*3(R,G,B)
        int r = rand()%igSource.Row();
        int c = rand()%igSource.Col();
        int ch = rand()%3;  //Channel Select ch =0 ->R_noise
        if(ch==0){
            //ch =0 ->R_noise
            igSource._ppA[r][c].r=255;
        }
        else if(ch==1){
            // ch =1 ->G_noise
            igSource._ppA[r][c].g=255;
        }
        else if(ch==2){
            // ch =2 ->G_noise
            igSource._ppA[r][c].b=255;
        }
    }
    return igSource;
}
KImageGray& KImageGray::GNoise(KImageGray &igSource){
    double mean=0, sig=0, sig2=0;
    for(int i=0; i<igSource.Row(); i++){
        for(int j=0; j<igSource.Col(); j++){
           mean= mean + igSource._ppA[i][j];    //get Mean
        }
    }
    mean/=igSource.Size();
    for(int i=0; i<igSource.Row(); i++){
        for(int j=0; j<igSource.Col(); j++){
           sig2= pow(igSource._ppA[i][j]-mean,2);   //Get Variance
        }
    }
    sig2/=igSource.Size();
    sig=sqrt(sig2);
    KGaussian r,m;              //Make Gaussian Noise (r= Image Histogram, m= normalized Histogram)
    double noise_kernel =5;
    m.Create(0,1);
    r.Create(mean,sig2);
    for(int i=0; i<igSource.Row();i++){
        for (int j=0; j<igSource.Col(); j++){
           double image_normal,zero_normal=0;
           r.OnRandom(mean);        //Turn on Make Random Intensity
           m.OnRandom();
           image_normal= r.Generate();  //Make Random Intensity
           zero_normal= m.Generate();   //Make Random Intensity
           double noise= (int)((image_normal/noise_kernel)*zero_normal)+igSource._ppA[i][j];
           if(noise>=255){
               igSource._ppA[i][j]=255;
           }
           else if(noise<=0){
               igSource._ppA[i][j]=0;
           }
           else igSource._ppA[i][j]=noise;
        }
    }
    return igSource;
}
KImageColor& KImageColor::BoxFilter(KImageColor& igSource,int ksize){

    KImageColor icTmp(*this);       //Out Image

    int nhalf= (ksize%2)+1;
    KMatrix mMask(nhalf*2 + 1, nhalf*2+1);      //Make Kernel
    double dR,dG,dB;
    int nSy= nhalf, nEy= Row()-nhalf;
    int nSx= nhalf, nEx= Col()-nhalf;
    double dscale;
    for(int r=-nhalf,i=0;r<=nhalf; i++,r++){
        for(int c=-nhalf,j=0; c<nhalf; j++,c++){
            mMask[i][j]= 1;
            dscale++;
        }
    }
    mMask/=dscale;          //Get BoxKernel Scale

    //Convolution
    for(int i=nSy; i<nEy;i++){
        for(int j=nSx; j<nEx;j++){
            dR= dG= dB =0.0;
                for(int r=-nhalf,rr=0;r<=nhalf; rr++,r++){
                    for(int c=-nhalf,cc=0; c<nhalf; cc++,c++){
                        dR +=icTmp._ppA[i+c][j+c].r*mMask[rr][cc];
                        dG +=icTmp._ppA[i+c][j+c].g *mMask[rr][cc];
                        dB +=icTmp._ppA[i+c][j+c].b *mMask[rr][cc];
                    }
                }
                igSource._ppA[i][j].r=(unsigned char)(dR);
                igSource._ppA[i][j].g=(unsigned char)(dG);
                igSource._ppA[i][j].b=(unsigned char)(dB);
        }
    }
...
KImageGray KImageGray::BinaryDilate5(const int& nType)
{
    KImageGray  igOut = *this;
            ...
            for(int j=2,jj=Col()-4; jj; j++,jj--)
            {
                if(_ppA[i][j]==0)
                    if(_ppA[i-2][j-2]!=0      ||_ppA[i-2][j-1]!=0      || _ppA[i-2][j]!=0||_ppA[i-2][j+1]!=0|| _ppA[i-2][j+2]!=0||
                            ...
                            _ppA[i+1][j-2]!=0 ||_ppA[i+1][j-1]!=0 ||_ppA[i+1][j]!=0 || _ppA[i+1][j+1]!=0 || _ppA[i+1][j+2]!=0||
                            _ppA[i+2][j-2]!=0 ||_ppA[i+2][j-1]!=0 ||_ppA[i+2][j]!=0 ||_ppA[i+1][j+1]!=0 ||_ppA[i+2][j+2]!=0)
                        igOut._ppA[i][j] = 255;
            }
    }
    else if(nType == _BACKGROUND)
...
KImageGray KImageGray::BinaryErode5(const int& nType)
{
    KImageGray  igOut = *this;
            ...
            for(int j=2,jj=Col()-4; jj; j++,jj--)
            {
                if(_ppA[i][j])
                    if(     _ppA[i-2][j-2]  ||_ppA[i-2][j-1]  || _ppA[i-2][j] ||_ppA[i-2][j+1]   ||_ppA[i-2][j+2]  ||
                            ...
                            _ppA[i+1][j-2]  ||_ppA[i+1][j-1]  ||_ppA[i+1][j]  || _ppA[i+1][j+1]  ||_ppA[i+1][j+2]  ||
                            _ppA[i+2][j-2]  ||_ppA[i+2][j-1]  ||_ppA[i+2][j]  ||_ppA[i+1][j+1]   ||_ppA[i+2][j+2] )
                        igOut._ppA[i][j] = 255;
            }
    }
...
void MG::Canny(int tl, int th, KImageGray& igin, KImageGray& igout){
    KStack<EP>::RemoveAll();
   _skDefinite.RemoveAll();
   _nW  = igin.Col();
   _nH  = igin.Row();
   _idMag.Create(_nH,_nW);
   _iwAng.Create(_nH,_nW);
   _igDir.Create(_nH,_nW);
   _idBuf.Create(_nH,_nW);
   double  dGradX,dGradY,dTmp;
   KMath   oMath;
    for (int i=_nHalf; i<_nH-_nHalf; i++){
        for(int j=_nHalf; j<_nW-_nHalf; j++){
            //Convolution
            dGradX = dGradY = 0.0;
            for(int r=-_nHalf,rr=0; r<=_nHalf; r++,rr++)
                 for(int c=-_nHalf,cc=0; c<=_nHalf; c++,cc++){
    ...
    //Orient
        if(_idMag._ppA[i][j]>tl){
            dTmp = oMath.Atan((float)dGradX,(float)dGradY); // 각도 구하기
            _iwAng._ppA[i][j]= (unsigned short)(dTmp+0.5);
            _igDir._ppA[i][j]= (unsigned short)((((int)(dTmp/22.5)+1)>>1)&0x00000003);// 0~3 방향
        }
        else _idMag._ppA[i][j] =0.0;

        }
   }
    // non-Maxima suppression
    int pointx[4] = {1,1,0,-1};
    int pointy[4] = {0,1,1,1};
    int difH=_nH-_nHalf-1, difW=_nW-_nHalf-1;
    EP oEdgeMag;
    for (int i=_nHalf; i<difH; i++){
        for(int j=_nHalf; j<difW;j++){
            if(_idMag[i][j]==0.0) continue;

            if(_idMag[i][j] >_idMag._ppA[i+pointy[_igDir._ppA[i][j]] ][j+pointx[_igDir._ppA[i][j]] ]
                    && _idMag._ppA[i][j]>_idMag._ppA[i-pointy[_igDir._ppA[i][j]] ][j-pointx[_igDir._ppA[i][j]] ])
            {
                if(_idMag._ppA[i][j]>th){
                    oEdgeMag.u = (unsigned short)j;
                    oEdgeMag.v = (unsigned short)i;
                    oEdgeMag.wAng = _iwAng._ppA[i][j];
                    oEdgeMag.wDir = _igDir._ppA[i][j];
                    oEdgeMag.dMag = _idMag._ppA[i][j];
                    _skDefinite.Push(oEdgeMag);
                }
                _idBuf._ppA[i][j] = _idMag._ppA[i][j];
            }
     }
    ...
    while(_skDefinite.Pop(oEdgeMag)!=-1){
        // 엣지 리스트 저장
        KStack<EP> ::Push(oEdgeMag);
        // 엣지 좌표 저장
        tmpx = oEdgeMag.u;
        tmpy = oEdgeMag.v;
        //Serach neighbor edges
        for(int i=-1; i<2;i++){
            for(int j=-1; j<2;j++){
                if(_idBuf._ppA[tmpy+i][tmpx+j] && _idBuf._ppA[tmpy+i][tmpx+j]<=th){
                                    oEdgeMag.u    = (unsigned short)(tmpx+j);
                                    oEdgeMag.v 	  = (unsigned short)(tmpy+i);
                                    oEdgeMag.wAng = _iwAng._ppA[tmpy+i][tmpx+j];
                                    oEdgeMag.wDir = _igDir._ppA[tmpy+i][tmpx+j];
                                    oEdgeMag.dMag = _idBuf._ppA[tmpy+i][tmpx+j];
                                    _skDefinite.Push(oEdgeMag);
                                    _idBuf._ppA[tmpy+i][tmpx+j] = 0.;
            }
        }
    }
   }
   ...
   // Image OutPut
   igout.Create(_nH,_nW);
   while(Pop(oEdgeMag)!= -1)
       igout._ppA[oEdgeMag.v][oEdgeMag.u]=255;
}
void MG::Gradient(const KRect& rcRegion,const KImageGray& igIn){
    //set dimension of image
    _nW = rcRegion.Width();
    _nH = rcRegion.Height();
    ...
    //compute magnitude and direction of edge
    for(int i=0,ii=rcRegion._nTop; i<_nH; i++,ii++){
        if(ii-_nHalf<0 || ii+_nHalf>igIn.Col()-1) continue;

        for(int j=0,jj=rcRegion._nLeft; j<_nW; j++,jj++){
             if(jj-_nHalf<0 || jj+_nHalf>igIn.Row()-1) continue;

             //convolution
             for(int r=-_nHalf,rr=0; r<=_nHalf; r++,rr++)
                for(int c=-_nHalf,cc=0; c<=_nHalf; c++,cc++){
                    _dGx[i][j] += igIn[ii+r][jj+c]*_mKernelX[rr][cc];
                    _dGy[i][j] += igIn[ii+r][jj+c]*_mKernelY[rr][cc];
    }}}
}
void MG::Create(double dSigma, int nLength, int nCapacity){
#ifdef _DEBUG
    assert(dSigma != 0.0);
#endif
    ...
    //
    KStack<EP>::Create(nCapacity);
    _skDefinite.Create(nCapacity); //temporal stack
    ...
    if(_nMaskLength == 0)
    {
      ...
      //set convolution mask length
      _nMaskLength   =  (_nMaskLength%2 == 0 ? _nMaskLength+1 : _nMaskLength);
      _nHalf         =  (_nMaskLength-1) / 2;
    }
...
int CH::Init(void *pInfo){
   _info = *(CircleHough_info*)pInfo ;
   //
   if(_aryVote.Pointer()){
       for(unsigned int i=0; i<_aryVote.Row(); i++){
           for(unsigned int j=0; j<_aryVote.Col(); j++){
               delete[] _aryVote[i][j];
           }
       }
   }

   _aryVote.Create(_info.nNumGridCy,_info.nNumGridCx);
   for(int i=0; i<_info.nNumGridCy; i++){
       for(int j=0; j<_info.nNumGridCx; j++){
           _aryVote[i][j]= new double[_info.nNumGridR];
           memset(_aryVote[i][j],0,_info.nNumGridR*sizeof(double));
       }
   }


   _dSx = (_info.nNumGridCx ==1 ?0 : (double)(_info.nRight-_info.nLeft)/
                                 (double)(_info.nNumGridCx-1));
   _dSy = (_info.nNumGridCy ==1 ?0 : (double)(_info.nBottom-_info.nTop)/
                                 (double)(_info.nNumGridCy-1));
   _dSr = (_info.nNumGridR ==1 ?0 : (double)(_info.nMaxR-_info.nMinR)/
                                 (double)(_info.nNumGridR-1));

   return 1;
}
void CH::Create(double dSigma,int nMaskLength,int nCapacity)
{
#ifdef _DEBUG
    assert(dSigma != 0.0);
#endif
    ...
    //
    //KStack<KEDGEPIXEL>::Create(nCapacity);
    _p.Create(nCapacity); //temporal stack
...
void CH::Canny(int tl, int th,KImageGray& igOut){
    //init. stack
    KStack<KEDGEPIXEL>::RemoveAll();
    _p.RemoveAll();
    //set dimension of image
    ...
    _idBuf.Create(_nHeight,_nWidth);
     ...
     for(i=_nHalf; i<_nHeight-_nHalf; i++){
         for(j=_nHalf; j<_nWidth-_nHalf; j++){
             ...
             for(r=-_nHalf,rr=0; r<=_nHalf; r++,rr++)
                 for(c=-_nHalf,cc=0; c<=_nHalf; c++,cc++){
                     dGradX += _igIn._ppA[i+r][j+c] * _mKernelX._ppA[rr][cc];
                     dGradY += _igIn._ppA[i+r][j+c] * _mKernelY._ppA[rr][cc];
                 }
             //magnitude
             _idMag._ppA[i][j] = _ABS(dGradX) + _ABS(dGradY);
             //direction
     ...
     for(i=_nHalf+1; i<nH; i++){
         for(j=_nHalf+1; j<nW; j++){
             ...
             if(_idMag._ppA[i][j] > _idMag._ppA[i+nShiftY[ _igDir._ppA[i][j] ]][j+nShiftX[ _igDir._ppA[i][j] ]] &&
                     _idMag._ppA[i][j] > _idMag._ppA[i-nShiftY[_igDir._ppA[i][j] ]][j-nShiftX[_igDir._ppA[i][j] ]]){
                 if(_idMag._ppA[i][j] > th){
                     oEdge.u    = (unsigned short)j;
                     oEdge.v 	  = (unsigned short)i;
                     oEdge.wAng = _iwAng._ppA[i][j];
                     oEdge.wDir = _igDir._ppA[i][j];
                     oEdge.dMag = _idMag._ppA[i][j];
                     _p.Push(oEdge);
                 }
                 _idBuf._ppA[i][j] = _idMag._ppA[i][j];
     }}}
     ...
     //hysteresis thresholding
     int iy,jx;
     while(_p.Pop(oEdge) != -1)
     {
         //store edge list
         KStack<KEDGEPIXEL>::Push(oEdge);
         //get edge coordinate
         jx  = oEdge.u;
         iy  = oEdge.v;
         //search neighbor edges
         for(i=-1; i<2; i++)
             for(j=-1; j<2; j++)
                 if(_idBuf._ppA[iy+i][jx+j] && _idBuf._ppA[iy+i][jx+j]<=th)
                 {
                     oEdge.u    = (unsigned short)(jx+j);
                     oEdge.v 	  = (unsigned short)(iy+i);
                     oEdge.wAng = _iwAng._ppA[iy+i][jx+j];
                     oEdge.wDir = _igDir._ppA[iy+i][jx+j];
                     oEdge.dMag = _idBuf._ppA[iy+i][jx+j];
                     _p.Push(oEdge);
                     _idBuf._ppA[iy+i][jx+j] = 0.;
                 }
     }
     ...
     int ucos=0;
     int vsin=0;
     igOut.Create(_igIn.Row(),_igIn.Col());
     while(Pop(oEdge)!= -1){
         u = oEdge.u;
         v = oEdge.v;
         if(u>(_nWidth/2)+15){
             //데이터 연산을 줄이기 위해서 대략적 코인 이미지가 있는
             //절반의 이미지에서 엣지데이터를 이용해서 원을 그린다.
            for(int i=0; i<24; i++){
                 int angle = (i+1)*15;
                 ucos = radius*cos(angle);
                 vsin = radius*sin(angle);
                 //원 그리는 함수
                 igOut._ppA[v+vsin][u+ucos]=255;
                 point[v+vsin][u+ucos]+=1;
                 // 원 주위가 몇번 출력 되었는지 기록하는 배열
            }
         }
         else continue;
    }
...
void CH::GetPoint(){
    ...
    for(int i=(a/2); i<a; i++){
        for(int j= (b/2);j<b; j++){
            //계산 시간 절약을 위해 이미지의 절반에서만 진행
            if(max< point[i][j]){
                max= point[i][j];
                maxu=j, maxv=i;
                //최대로 많이 voting 좌표 i,j
                //그 횟수 = max
            }
            else continue;
        }
...
void CH::GetCenter(KImageGray& igout){

    int u=0, v=0; double Angle=0;
    for(int i=0; i<360; i++){
        //Angle = (i/180)*M_PI;
        //중심을 주위로 반지름이 51px인 원을 그림
        u = maxu+radius*cos(i);
        v = maxv+radius*sin(i);
        igout._ppA[v][u]=255;
    }
    //아래는 원의 중심을 표시하기 위한 구문
    igout._ppA[maxv][maxu]=255;
    igout._ppA[maxv+1][maxu]=255;
    igout._ppA[maxv-1][maxu]=255;
    igout._ppA[maxv][maxu+1]=255;
    igout._ppA[maxv][maxu-1]=255;
}

void CH::GetPlugEdge(int tl, int th, KImageGray& igOut){
            //init. stack
            KStack<KEDGEPIXEL>::RemoveAll();
            _p.RemoveAll();
            //set dimension of image
            ...
            _idBuf.Create(_nHeight,_nWidth);
             ...
             while(Pop(oEdge)!= -1){
                 ...
                 v = oEdge.v;
                 if(u<(_nWidth/2)+15){
                     //데이터 연산을 줄이기 위해서 대략적 플러그 이미지가 있는
                     //절반의 이미지에서 엣지데이터를 이용해서 플러그를 그린다.
                     //엣지좌표와 모델 중심 좌표를 좌표 이동을 통해 얻어진 이동 값을
                     //바탕으로 모델의 모든 좌표를 이동시킨다.
                     // 중심점과 엣지 데이터의 크기는 4가지 종류가 나오므로 if문으로 다르게 계산.
                     int dx=0,dy=0;
                     for (int i=0; i<count; i++){
                         if(v>ymean){
                             if(u>xmean){
                                 dx=u-xmean;
                                 dy=v-ymean;
                                 igOut._ppA[yarray[i]+dy][xarray[i]+dx]=255;
                                 point[yarray[i]+dy][xarray[i]+dx]+=1;
                             }
                             else
                             {  dx=xmean-u;
                                dy=v-ymean;
...
void CH::GetPlugPoint(){

    int a = _igIn.Row();
    int b = _igIn.Col();

    for(int i=0; i<(a/2)+30; i++){
        for(int j=0;j<b/2; j++){
            //계산 시간 절약을 위해 이미지의 절반에서만 진행
            if(max< point[i][j]){
                max= point[i][j];
...
void CH::GetPlugPicture(KImageGray& igOut){
    int dx=0,dy=0,count=86;
    for (int i=0; i<count; i++){
        if(maxv>ymean){
            if(maxu>xmean){
                dx=maxu-xmean;
                dy=maxv-ymean;
                igOut._ppA[yarray[i]+dy][xarray[i]+dx]=255;     //plug 엣지 표시
                igOut._ppA[yarray[i]+dy-1][xarray[i]+dx-1]=255; //plug엣지를 두껍게 만들어주기
            }
            else
            {  dx=xmean-maxu;
               dy=maxv-ymean;
               igOut._ppA[yarray[i]+dy][xarray[i]-dx]=255;
               igOut._ppA[yarray[i]+dy-1][xarray[i]-dx+1]=255;
            }
        }
        else {
            if(maxu>xmean){
    ...
    igOut._ppA[maxv][maxu]=255; // plug 중심 표시
}
