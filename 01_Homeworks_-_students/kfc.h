...
public:
    ...
    KImageGray&  HistoMatching(KImageGray& igSource, KImageGray& igTarget, KImageGray de);
    KImageGray&  SPNoise(KImageGray& igSource);
    KImageGray&  GNoise(KImageGray& igSource);
    ...
    KImageGray   BinaryDilate5(const int& nType);
    KImageGray   BinaryErode(const int& nType);
    KImageGray   BinaryErode5(const int& nType);
...
public:
    ...
    KImageColor&    SPNoise(KImageColor& igSource);
    KImageGray&     GNoise(KImageGray& igSource);
    KImageColor&    BoxFilter(KImageColor& igSource, int ksize);
...
typedef struct tagEP{
    unsigned short u;
    unsigned short v;
    unsigned short wAng;        // 0~359
    unsigned short wDir;        // 0,1,2,3
    double dMag;
}EP;

class MG : public KStack<EP> {
    KMath            _oMath;
    KMatrix			 _mKernelX, _mKernelY;  //convolution mask
    KImageDouble     _idMag, _idBuf;         //buffer for edge gradients and magnitudes
    KImageWord		 _iwAng;                //0~360
    ...
    int              _nHalf, _nMaskLength=0;  //dimension of convolution mask
    int              _nW, _nH;     //image size
    KRect            _rcArea;
    double           _dSigma;
    double           dTh,dTL,dMag;
    KImageDouble    _dGx, _dGy;
    KStack<EP>      _skDefinite;
    unsigned int	   _dwpSQR[111];
               //edge stdev.
public:
    MG(){ };
    MG(double dSigma,int nLength=0,int nCapacity=2000){ Create(dSigma,nLength,nCapacity); }
    ~MG(){};
    void Canny(int tl, int th, KImageGray& Igin, KImageGray& Igout);
    void Gradient(const KRect& rcRegion,const KImageGray& igIn);
    void Create(double dSigma,int nLength=0,int nCapacity=2000);
};

typedef struct tagCircleHough{
    int nLeft,nRight,nTop,nBottom;
    int nMinR,nMaxR,nNumGridCx,nNumGridCy;
    int nNumGridR;
    int nDirEdge; // Out 0 In 1
    int NThreshVote;
}CircleHough_info;
class CH : public KStack<KEDGEPIXEL>
{
    KMatrix			 _mKernelX, _mKernelY;  //convolution mask
    KImageDouble      _idGradX, _idGradY;
    KImageDouble      _idMag,_idBuf;         //buffer for edge gradients and magnitudes
    ...
    int point[600][600];
    //edge stdev.
    CircleHough_info     _info;
    KArray<double*> _aryVote;
    KImageGray      _igIn;
    double          _dSx;
    double          _dSy;
    double          _dSr;
    KStack<KEDGEPIXEL>       _p;
    int max = 0, maxu=0, maxv=0;
    unsigned int	   _dwpSQR[111];
    double radius;
    int xarray[100]={0,};
    int yarray[100]={0,};
...
public :
    CH(){}
    CH(KImageGray In, double R){_igIn = In;
                            Create(0.7,0,2000);
                            radius= R/2;}
    CH(KImageGray In, int x[], int y[], int a ,int b){_igIn = In;
                                            Create(0.7,0,2000);
                                            xmean=a; ymean=b;
                                            for(int i=0; i<100; i++){
                                                xarray[i]=x[i];
                                                yarray[i]=y[i];
                                                // txt 좌표를 class 변수 배열에 저장
                                            }}
    //
    void Create(double dSigma,int nMaskLength,int nCapacity);
    ~CH(){};
    int Init(void* pInfo);

    void* Info(){return &_info;}
    void Canny(int tl, int th,KImageGray& igOut);
    void GetPoint();
    void GetCenter(KImageGray& igOut);
    void GetPlugEdge(int tl, int th,KImageGray& igOut);
    void fileread(std::istream readtxt, KImageGray& Model);
    void GetPlugPoint();
    void GetPlugPicture(KImageGray& igOut);
};
